...
## v3.6.0：与 Gitee 图像工具联动

配合 **Gitee AI Image >=v5.2.0**，群聊中可以把短期历史图片作为单图编辑输入或自拍额外参考。例如“把 Alice 刚才的猫改成水彩”“抱着上图的猫自拍”“把刚生成的那张背景改成海边”。实际自然语言选图由聊天模型决定；明确引用和发送者优先，不默认使用群里最后一张。

- **受控引用**：目录提供图片 ID、发送者、消息位置，以及生成结果的任务和请求者；Gitee 按 ID 取缓存保留的数据，不接收任意文件路径，不用视觉预览副本替代编辑输入。保留数据可能已被 Core 或图片压缩预处理，并不保证等同原始上传字节。
- **参考用途**：编辑的主体、画风、服装、动物或物体、姿势、背景各有明确角色；自拍固定人物身份仍由 Gitee 管理，额外参考不改写永久自拍设置。
- **任务独立**：Gitee 接单就保存输入副本；任务执行不依赖之后的图片缓存。成功发送的单图结果可登记到目录，失败发送不登记。
- **多人接续**：结果记录原请求者、conversation、任务和父图片；个人最近结果按任务发起顺序判断，避免慢任务晚完成覆盖新任务。其他群友可明确引用同会话可见的结果，不跨会话取图。
...
- `get_image_catalog(event)`：当前会话请求快照中的目录。
- `resolve_reference_images(event, image_ids)`：校验当前快照和 epoch，全部成功才返回 bytes、SHA256 与来源；最多 8 张、单图 20 MiB、合计 64 MiB，独立于聊天看图上限。
- `capture_image_request(event, conversation_id)`：任务开始前捕获实例与会话凭据。
- `register_generated_image(receipt, task_id=..., data=..., parent_ids=...)`：已成功交付单图的短期登记；重复任务只能幂等返回同一内容，不能覆盖已公开 ID。

可在真实 AstrBot 环境运行 `python tests/verify_two_plugin_bridge.py /path/to/context_plugin /path/to/gitee_plugin`，验证预处理清理、跨用户取图、任务输入副本、自拍参考与结果接续。该集成使用替代的生图/发送函数，不会向群发图或调用付费生图。
