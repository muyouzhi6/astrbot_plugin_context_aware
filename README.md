...
修复 v3.5.0 在真实群聊中“索引有图片，但工具返回 0 张图片”的问题：AstrBot 会在消息结束时清理预处理生成的临时文件，现在插件会在事件结束前把本地图片接管到有界内存缓存。远程 URL 仍采用后台预取，本地接管每条消息最多等待 3 秒。

升级后请使用新发送的图片验证；旧版本已经丢失的临时图片无法恢复。
